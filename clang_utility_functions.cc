...
bool is_packet_func(const clang::FunctionDecl *func_decl) {
  ...
  return func_decl->getNumParams() == 1 and
         func_decl->getParamDecl(0)->getType().getAsString() == "struct Packet";
}

std::string
replace_var_helper(const Expr *expr,
...
std::set<std::string>
identifier_census(const clang::TranslationUnitDecl *decl,
  ...
  for (const auto *child_decl : dyn_cast<DeclContext>(decl)->decls()) {
    ...
    if (isa<RecordDecl>(child_decl)) {
      ...
      if (var_selector.at(VariableType::FUNCTION_PARAMETER)) {
        ...
        for (const auto *parm_decl :
             dyn_cast<FunctionDecl>(child_decl)->parameters()) {
          identifiers.emplace(dyn_cast<ParmVarDecl>(parm_decl)->getName());
        }
      }
    } else if (isa<VarDecl>(child_decl)) {
      const auto *underlying_type =
          dyn_cast<VarDecl>(child_decl)->getType().getTypePtrOrNull();
      ...
      } else if (isa<BuiltinType>(underlying_type) and
                 dyn_cast<BuiltinType>(underlying_type)->isInteger()) {
        if (var_selector.at(VariableType::STATE_SCALAR)) {
          identifiers.emplace(dyn_cast<VarDecl>(child_decl)->getName());
        }
      } else {
        throw std::logic_error(
            "We don't support this: state variable " +
            clang_value_decl_printer(dyn_cast<VarDecl>(child_decl)) +
            " has type " +
            std::string(dyn_cast<VarDecl>(child_decl)->getName()));
      }
    } else {
...
std::set<std::string> gen_var_list(const Stmt *stmt,
  ...
  if (isa<CompoundStmt>(stmt)) {
    ...
    if (if_stmt->getElse() != nullptr) {
      return gen_var_list(if_stmt->getCond(), var_selector) +
             gen_var_list(if_stmt->getThen(), var_selector) +
             gen_var_list(if_stmt->getElse(), var_selector);
    } else {
      return gen_var_list(if_stmt->getCond(), var_selector) +
             gen_var_list(if_stmt->getThen(), var_selector);
    }
    ...
    return gen_var_list(bin_op->getLHS(), var_selector) +
           gen_var_list(bin_op->getRHS(), var_selector);
  } else if (isa<ConditionalOperator>(stmt)) {
    const auto *cond_op = dyn_cast<ConditionalOperator>(stmt);
    return gen_var_list(cond_op->getCond(), var_selector) +
           gen_var_list(cond_op->getTrueExpr(), var_selector) +
           gen_var_list(cond_op->getFalseExpr(), var_selector);
    ...
    if (var_selector.at(VariableType::STATE_ARRAY)) {
      return std::set<std::string>{
          clang_stmt_printer(dyn_cast<ArraySubscriptExpr>(stmt)->getBase())};
    } else if (var_selector.at(VariableType::PACKET)) {
      return std::set<std::string>{
          clang_stmt_printer(dyn_cast<ArraySubscriptExpr>(stmt)->getIdx())};
    } else {
      return std::set<std::string>();
    }
    ...
    throw std::logic_error("gen_var_list cannot handle stmt of type " +
...
std::string gen_pkt_fields(const TranslationUnitDecl *tu_decl) {
  ...
  for (const auto &field :
       identifier_census(tu_decl, {{VariableType::PACKET, true},
...
std::string replace_vars(const clang::Expr *expr,
  ...
  if (isa<ParenExpr>(expr)) {
    return "(" +
           replace_vars(dyn_cast<ParenExpr>(expr)->getSubExpr(), repl_map,
                        var_selector) +
           ")";
    ...
    const auto cond_str =
        replace_vars(cond_op->getCond(), repl_map, var_selector);
    const auto true_str =
        replace_vars(cond_op->getTrueExpr(), repl_map, var_selector);
    const auto false_str =
        replace_vars(cond_op->getFalseExpr(), repl_map, var_selector);
    ...
    if (var_selector.at(VariableType::STATE_SCALAR))
      return replace_var_helper(expr, repl_map);
    else
    ...
    if (var_selector.at(VariableType::STATE_ARRAY))
      ...
      return clang_stmt_printer(array_op->getBase()) + "[" +
             replace_vars(array_op->getIdx(), repl_map, var_selector) + "]";
    else
      return clang_stmt_printer(expr);
    ...
    throw std::logic_error("replace_vars cannot handle expr " +
                           std::string(clang_stmt_printer(expr)) + " of type " +

...
ternary_body(const clang::CompoundStmt *function_body,
  ...
  for (const auto *child : function_body->children()) {
    ...
    if (isa<ConditionalOperator>(rhs)) {
      ...
      if (clang_stmt_printer(cond->getCond()->IgnoreParenImpCasts()) == "1") {
        ...
        transformed_body +=
            "  " + clang_stmt_printer(lhs) + " = " +
            clang_stmt_printer(cond->getTrueExpr()->IgnoreParenImpCasts());
        transformed_body += ";\n";
      } else {
        if (isa<ConditionalOperator>(
                prev_bin_op->getRHS()->IgnoreParenImpCasts())) {
          const auto *prev_cond = dyn_cast<ConditionalOperator>(
              prev_bin_op->getRHS()->IgnoreParenImpCasts());
          if (clang_stmt_printer(prev_cond->getCond()->IgnoreParenImpCasts()) ==
            ...
            /*if (clang_stmt_printer(
                    prev_bin_op->getLHS()->IgnoreParenImpCasts()) ==
                clang_stmt_printer(bin_op->getLHS()->IgnoreParenImpCasts())) {
                  // they have the same LHS.
                  if (clang_stmt_printer(prev_cond->getFalseExpr()))
                }
              // if (clang_stmt_printer(prev_cond->getTrueExpr()) ==
              // clang_stmt_printer(prev_bin_op->getLHS())) {

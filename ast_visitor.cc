...
std::string AstVisitor::expr_visit_transform(clang::ASTContext *c,
                                             const Stmt *e) {
  this->ctx = c;
...
std::string AstVisitor::ast_visit_un_op(const UnaryOperator *un_op) {
  ...
  // assert_exception(opcode_str == "!"); // ruijief note: this is here probably
  // becuase Domino ALU might not handle expressions other than '!'.
